...
def plot_bandpass(data, ax):
    ...
    coefs = np.polyfit(range(len(band)), band, 32)
    p = np.poly1d(coefs)
    print coefs

    spectra = np.sum(data, axis=1)
    print np.median(spectra), np.std(spectra)

    ax.plot(x[::-1], np.sum(data, axis=0) / (nsamp * integs * 1.0), 'r', x[::-1], p(x[::-1]), 'b--')
    ax.plot(x[::-1], (np.sum(data, axis=0) / (nsamp * integs * 1.0)) - p(x[::-1]), 'g-')
    ax.grid(True)
