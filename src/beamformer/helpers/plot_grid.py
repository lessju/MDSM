...
if __name__ == "__main__":
    ...
    p.add_option('-s', '--gridsize', dest='grid', type='int', default=5,
        help='Beam grid size')
...
#        files[number] = f
    ...
    # Prepare grid
    f, axarr = plt.subplots(opts.grid, opts.grid, sharex=True, sharey=True)
    ...
    for k, v in files.iteritems():
        ...
        print "Beam %d in (%d, %d)" % (k, k / opts.grid, k % opts.grid)
        axarr[k / opts.grid][k % opts.grid].plot(x, data)
