...
# Create regular expression search string
searchString = 'nsamp: (?P<nsamp>\d*), nchans: (?P<nchans>\d*),.*tdms: (?P<tdms>\d*),.*\n\
Copied to GPU in: (?P<togpu>.*)\nProcessed in: (?P<time>.*)\n\
Copied from GPU in: (?P<fromgpu>.*)\nTotal time: \d*'
...
if len(values1) > 1:
    ...
    for value1 in values1:
        matchedIter = re.finditer(searchString, fileData)
        x, y = [], []
