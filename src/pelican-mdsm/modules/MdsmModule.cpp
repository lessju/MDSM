...
void MdsmModule::run(DataBlob* incoming, DedispersedTimeSeriesF32* dedispersedData)
{
    ...
    float blobMean = 0.0, blobRMS = 0.0;
    ...
    {        
        ...
        // Compute where we are in the array of means and rmss, depending on whether 
        // we have a maxhift bit at the front
    ...
    if (blobMean != 0.0) { _bufferMean += blobMean; _bufferRMS += blobRMS;    }
    _samples += copySamp;
    _gettime = _samples;
